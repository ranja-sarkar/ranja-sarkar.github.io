...
So we are back to correlation (corr), the measure with which we started the topic here. We shall see how correlation and covariance (cov) are similar as well as different. Well, covariance is unstandardized version of correlation. Correlation is not only a measure of how two variables move wrt each other, but also of how strongly those two are related and its value lies between -1 and +1.
...
1. **Data Model**, wherein the data-generating mechanism is assumed to be known and model parameters are estimated from the data

2. **Algorithmic Model**, where the data-generating mechanism is unknown (may be stochastic/non-deterministic) and uses an algorithm to find a function f(x) that operates on variables x in the data to estimate a response y.
