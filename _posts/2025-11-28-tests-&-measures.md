...
The **law of large numbers** states that when a collection of random variables is a large sample, the **sample mean converges to true value** (or expected mean if it exists). The law applies to the mean obtained from a large number of repeated trials and claims that this value converges to the expected value. For a small sample, **there's a difference between these values, called the error**.
